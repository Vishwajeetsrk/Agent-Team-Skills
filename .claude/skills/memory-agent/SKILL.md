...
One-time setup: create a small private repo (e.g. `github.com/Vishwajeetsrk/agent-memory`), point `REPO_URL` in the script at it, run `pull` once to clone it into `~/.agent-memory`. After that, pull/push is the entire ritual — in Claude Code, in claude.ai's bash tool, or on a second machine, all reading and writing the same repo. Without this step, treat memory as session-local and say so plainly rather than assuming it carried over.
